...
| [Lighter](https://www.walmart.com/ip/Ozark-Trail-Multipurpose-BBQ-Lighter-Red-and-Black-2-Pack/1450295214?athbdg=L1200&from=/search) | Used for melting the cut ends of paracord so they don't keep fraying and coming loose | $2.24 | 1 | $2.24 |
...
And then to help you visualize roughly how each component was soldered together, I made the following schematic for our presentation and figure I would include it here. Just to make a small note that the schematic doesn't show, I ended up using two small pieces of perfboard to create a sort of 'hub' for each component that needed to share a specific wire to connect to. In other words, the components that needed a ground, 3.3V, Serial Clock Line (SCL), and Serial Data Line (SDA) wire were all connected to these perfboard 'hubs'. These connections were all done on a single piece of perfboard, with a second piece being used in order to connect the 3.3V to 5V converter board. During testing, I found that the Arduino couldn't recognize the micro-SD card nor board when everything was running off of 3.3V and when I tried using 5V on a whim everything worked flawlessly. Depending on what micro-SD board, this may not be an issue for you but if it is I suggest saving yourself a possible headache and try this to see if it works!

<img src=Reference-Photos/Payload-Electronics-Schematic.jpg width='800'>
...
Then, after watching several videos on high-altitude balloon launches and coming up with our list of criteria for optimal launch conditions, we devised a set of instructions for us to follow in order to have a quick, accident free launch:
  ...
  5. Attach the pressure regulator to the bottle with wrenches. Make sure to put teflon tape on the threads to prevent leaking if the regulator doesn't have a flared end. Also do not open the helium bottle yet
...
The biggest inspiration that I found that helped immensely with conducting this project and designing this github for other interested individuals and future engineering students was a fellow individual by the name of [blazerunner44](https://github.com/blazerunner44/Arduino-High-Altitude-Balloon?tab=readme-ov-file#user-content-balloon-payload). I loved the work he did and I tried my best to emulate his project despite my more limited skillset.
